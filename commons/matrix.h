...
template <typename T> struct vec<2,T> {
    ...
    vec<2,T> operator -(const vec<2,T> &V) const { return vec<2,T>(x-V.x, y-V.y); }
    vec<2,T> operator *(float f)          const { return vec<2,T>(x*f, y*f); }
...
template <typename T> struct vec<3,T> {
    ...
    vec(mat<4, 4, T> m): x(m[0][0]/m[3][0]), y(m[1][0]/m[3][0]), z(m[2][0]/m[3][0]) {};
    ...
    vec<3,T> operator -(const vec<3,T> &v) const { return vec<3,T>(x-v.x, y-v.y, z-v.z);}
    vec<3,T> operator *(float f)          const { return vec<3,T>(x*f, y*f, z*f);}
    T       operator *(const vec<3,T> &v) const { return x*v.x + y*v.y + z*v.z;}
...
public:
    ...
    static mat<DimRows,DimCols,T> identity(int dimensions) {
        ...
        for (size_t i=dimensions; i--; )
            for (size_t j=dimensions;j--; ret[i][j]=(i==j));
        return ret;
    }
